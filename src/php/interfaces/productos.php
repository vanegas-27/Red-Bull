...
$query = $bd -> read("redbull","articulos");
...
if(isset($_GET['categoria'])){
    ...
    $resul = $bd -> search("redbull","articulos",["categoria",$categoria]);

    while($row = mysqli_fetch_array($resul)){
        $nodos .="
        <article class='col-12 col-sm-6 col-xl-4 container-article scale-up-br'>
            <div>
                <img src='../../../".$row[3]."' alt='Foto de producto' class='img-fluid'>
            </div>
            <div class='container-parrafo'>
                <h5 class='price'>".$row[2]."</h5>
                <p>".$row[4]."</p>
                <span>".$row[1]."</span>
            </div>
        </article>\n";
